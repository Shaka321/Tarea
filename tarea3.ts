...
class User {
    constructor(public UserID: string, public name: string, public membership: MembershipType) { 
        this.loanHistory = []; // Inicializar el historial de préstamos vacío
    }
    public loanHistory: { bookTitle: string, borrowedDate: Date }[]; // Historial de préstamos del usuario
}
...
class Library implements ILoadManager {
    ...
    loadBook(book: Book, user: User): void {
        if (book.isAvailable) {
            ...
            user.loanHistory.push({ bookTitle: book.title, borrowedDate: book.borrowedDate });
...
user1.loanHistory.forEach(loan => {
    console.log(`Libro: ${loan.bookTitle}, Fecha de préstamo: ${loan.borrowedDate}`);
});
